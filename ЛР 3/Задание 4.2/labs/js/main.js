	...
	var trigger=0, name, age, sex, Data, education, mathematic, asu, cutting, programming,is; // обьявляем переменные которые понадобятся в ходе работы
		...
		function test(){ // Функция которая опрашивает пользователя
			...
			programming = confirm("Вы знаете программирование?");
			phisic =  confirm("Вы знаете физики?");
			...
			document.getElementById("phisic").setAttribute('disabled', 'disabled');
			writeToPage();	
		}
		function writeToPage(){ // функция которая записывает значение из наших переменных в элементы на странице
			...
			*/
			...
			document.getElementById("phisic").checked = phisic;
			trigger=1;
			myFunction();
		}
		...
		function myFunction() { // эта функция автоматически проверяет подходит ли наш кандидат: "кандидаты со знанем АСУ ТП и высшим
					//образованием".
	    	var x = document.getElementById("div1");В
		if (trigger==1) {
		if (document.getElementById("mathematic").checked ==true && document.getElementById("phisic").checked ==true)
		{
